...
| 前端         | React SPA (Babel-standalone) + Tailwind CDN | 瀏覽器內轉譯，無需 Node.js 建置；SPA 於 `/`，Jinja 僅剩 login |
...
| 雲端 MQTT    | Mosquitto                  | 輕量（~15 MB idle）、支援帳密認證與 TLS            |
...
|   |-- __init__.py
|   |-- main.py                      # FastAPI 入口 + SPA / login/logout（舊 Jinja 路由 301→/）
...
|   |       |-- pages/               # split 2026-07-16
|   |       |   |-- alerts.jsx · monitor.jsx · status.jsx · weather.jsx
|   |       |   +-- handover.jsx · audit.jsx · pumps.jsx
|   |       |-- icons.jsx · data.jsx · tweaks-panel.jsx
