...
2. **Configure** → Root Directory 填 `deploy/emqx`
3. 部署完成後，到 **Settings** → Service Name 改為 `emqx`
4. **Networking** 頁面：
   - 新增 `1883` (TCP) → 開啟 **Public**（給 Pi 連入）
   - 新增 `18083` (HTTP) → 開啟 **Public**（EMQX 管理介面）
5. 記下 **Private** hostname（如 `sdprs-xxx.zeabur.internal`）和 **Public** TCP 地址

**方法 B：Prebuilt Image**

1. 點擊 **新增服務** → **Prebuilt Image** → 填入 `emqx/emqx:5.8`
2. 同上設定 Networking

**EMQX Dashboard：** `https://<your-emqx-domain>.zeabur.app`，預設帳號 `admin` / `public`（登入後請改密碼）

> **關鍵：** Zeabur 內部服務名 ≠ Service Name。
> Central Server 的 `MQTT_BROKER` 必須填 **Private hostname**（如 `sdprs-xxx.zeabur.internal`），不是 `emqx`。
> Pi（外部網路）必須使用 EMQX 的 **Public TCP 轉發地址**（如 `<emqx-public-ip>:<port>`）。

> **MQTT 安全設定** 詳見 [../../deploy/MQTT_SECURITY.md](../../deploy/MQTT_SECURITY.md)。
...
| MQTT 連線 timeout（Pi 端）                                                                                    | `mqtt_broker` 用了 Zeabur 內部名              | Pi 需使用 EMQX 的**Public** TCP 轉發地址                                     |
| Central Server MQTT 連不上                                                                                    | `MQTT_BROKER=emqx` 但實際 hostname 不同       | 用 Networking 頁面的**Private** hostname（如 `sdprs-xxx.zeabur.internal`） |
...
- [ ] 主控台（`/`）顯示節點: 1/1
- [ ] EMQX Dashboard Clients 顯示 2 個連線（Pi + Central Server）
...
- **MQTT TLS：** 中央伺服器另有 `MQTT_USE_TLS` 環境變數（預設 `False`，雲端部署可設為 `True`）。Zeabur TCP 端口轉發為純 TCP，不含 TLS 包裝；若需 TLS，改用 EMQX WebSocket over HTTPS（port 8083 → Zeabur HTTP 代理）。
- **MQTT 端口穩定性：** EMQX 對外 TCP 端口（如 `:34567`）可能在重新部署後變更，屆時需更新 Pi `config.yaml` 的 MQTT 端口。改用 MQTT over WebSocket（EMQX 的 `:8083`，透過 Zeabur HTTP 代理，端口固定為 443）可避免此問題。
