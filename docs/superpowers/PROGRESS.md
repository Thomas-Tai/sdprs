...
**Last updated:** 2026-07-14 — *after* the PG-safe server slice (`e73718d`: T1 request paths + snapshot-GET auth + instant-ONLINE push) and the audience-based docs reconstruction (1,986-line README → 17-file `docs/` tree).
**Repo:** `sdprs/` (its own git repo; parent folder is not versioned) · **Remote:** `github.com/Thomas-Tai/sdprs`
**Current branch/commit:** `main` @ `e73718d` + the docs-restructure commit carrying this file — both pushed to `origin/main`.
...
| **`main`** | @ `e73718d` + the docs-restructure commit, **pushed to `origin/main`**. Carries pump reconstruction + V2 SPA + all 2026-07-13 theme slices + the 2026-07-14 PG-safe server slice + the audience-based docs tree. |
...
| **Tests** | **287 passing** (48 pump + 115 server + 124 glass). Zero failures. |
...
## 3. Verified test status (run 2026-07-14, on `main` @ `e73718d`)

| Suite | Command | Result |
|---|---|---|
| edge_pump | `cd edge_pump && python -m pytest tests -q` | **48 passed** |
| central_server | `cd central_server && python -m pytest tests -q` | **115 passed** (36 warnings) |
| edge_glass | `cd edge_glass && python -m pytest tests -q` | **124 passed** |
| **Total** | | **287 passed, 0 failed** |

Environment: Python **3.14.3**, pytest **9.0.2**, FastAPI **0.135.1**. (edge_glass +15 from non-blocking capture, +2 from glass-LWT, +11 from T3, +33 from T6. central_server +4 glass-LWT, +4 batch-cycles, +9 T5, +18 T2 auth, +7 T4, **+23 from the 2026-07-14 server slice** (dual-backend dispatch · snapshot auth · online-recovery broadcast). The SPA `app.jsx`/`api.jsx` changes + the `edge_glass_main.py` async wiring have no automated test — verified by diff review + a runtime integration smoke test.)
...
| # | Theme | Status | Detail |
|---|---|---|---|
| 1 | **Unify data-access layer** | 🟢 **Request paths closed** (`e73718d`) | Was: `get_db()` raises under PG → dashboard/alert-response call sites **500'd in PG mode**. Now: `get_backend()` accessor + dual-backend `list_events`/`get_event_counts`/`acknowledge_event`/`resolve_event` + 3 dashboard helpers; all 6 bypass sites in `main.py`/`api/alerts.py` unified — **no production request path calls `get_db()` under PG**. Remaining before a real cloud cutover: retention `datetime()` PG-portability (ties to T4) + a live-PG smoke test (dispatch is unit-tested; never run against real PostgreSQL). |
