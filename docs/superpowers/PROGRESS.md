...
**Last updated:** 2026-07-13 — *after* the pump-merge merge/push, the Theme-6 glass-hardening + detector-health slices, the Theme-4 data-lifecycle slice, the Theme-2 auth-hardening slice, **and the Theme-5 observability slice**.
**Repo:** `sdprs/` (its own git repo; parent folder is not versioned) · **Remote:** `github.com/Thomas-Tai/sdprs`
**Current branch/commit:** `main` @ `9d3f063` (**1 ahead of `origin/main` @ `7a4c57f`** — T5 observability slice `9d3f063` not yet pushed; T2 slice `c4660d8` + its docs `7a4c57f` already pushed)
...
| **Theme-5 observability** | ✅ Dashboard request-storm coalescing (debounce + in-flight guard), WS broadcast HOL-blocking fix (concurrent gather + per-client timeout), offline-mark TOCTOU false-alarm fix (`9d3f063`, local). Remainder: glass LWT, live node-health WS push (`broadcast_node_status` dead code), N+1 `/cycles` batch endpoint. |
| **Tests** | **225 passing** (48 pump + 81 server + 96 glass). Zero failures. |
...
Environment: Python **3.14.3**, pytest **9.0.2**, FastAPI **0.135.1**. (central_server +9 from the T5 slice: WS-broadcast isolation, offline-detection re-check; +18 earlier from T2 auth; +7 from T4. edge_glass +33 from T6. The SPA `app.jsx` change has no automated test — verified by review.)
...
3. ~~Theme 6 silent detector-blinding~~ ✅ **DONE** (`22e6084`+`3c110a2`). ~~Theme 4 retention/pruning~~ ✅ **DONE** (`dddda76`). ~~Theme 2 auth-hardening (bulk)~~ ✅ **DONE** (`c4660d8`). ~~Theme 5 request-storm + WS HOL + offline TOCTOU~~ ✅ **DONE 2026-07-13** (`9d3f063`). **Next candidates:** Theme 6 remainder (blocking MP4 encode on the capture loop, single-sensor fallback design, buffer arithmetic) and Theme 3 glass offline-autonomy (blocking initial `connect()`, event-data-destroyed-on-cleanup, 4xx tight-retry).
