...
**Current branch/commit:** `main` @ `525452e` (**1 ahead of `origin/main` @ `c34a173`** — batch pump-cycles N+1 fix `525452e` not yet pushed; T3 slice `73b5af0` + its docs `c34a173` already pushed)
...
| **Theme-5 observability** | ✅ Dashboard request-storm coalescing (debounce + in-flight guard), WS broadcast HOL-blocking fix, offline-mark TOCTOU fix (`9d3f063`, pushed), **`/cycles` N+1 → batch endpoint** (`525452e`, local). Remainder: glass LWT, live node-health WS push (`broadcast_node_status` dead code). |
| **Theme-3 glass autonomy** | ✅ Upload gives up on 4xx / after max retries (was infinite 1s retry), MP4-missing → FAILED not false-UPLOADED, non-blocking MQTT connect at boot (`73b5af0`, pushed). Remainder: **blocking MP4 encode + buffer arithmetic** (deferred — camera-thread-safety + hardware-timing, needs design pass). |
| **Tests** | **240 passing** (48 pump + 85 server + 107 glass). Zero failures. |
...
| central_server | `cd central_server && python -m pytest tests -q` | **85 passed** (deprecation warnings) |
| edge_glass | `cd edge_glass && python -m pytest tests -q` | **107 passed** |
| **Total** | | **240 passed, 0 failed** |

Environment: Python **3.14.3**, pytest **9.0.2**, FastAPI **0.135.1**. (central_server +4 from the batch-cycles endpoint; +9 from T5, +18 from T2 auth, +7 from T4. edge_glass +11 from T3, +33 from T6. The SPA `app.jsx`/`api.jsx` changes have no automated test — verified by review.)
...
| 5 | **Make observability real** | 🟡 **Advancing** (`525452e`) | ✅ MQTT-thread WS loop-capture fixed (pump card live). ✅ glass heartbeat emits `visual_health`/`audio_health` + throttled can-never-alert WARNING (`22e6084`), **server/dashboard consume + surface them** (`3c110a2`). ✅ **2026-07-13 (`9d3f063`):** SPA request-storm fixed (300ms debounce + in-flight guard); WS broadcast **head-of-line blocking** fixed (concurrent `gather` + 5s per-client timeout); offline-mark **TOCTOU false alarm** fixed (re-validate under lock). ✅ **`/cycles` N+1 collapsed to one `/api/pumps/cycles` batch call** (`525452e`). 🔴 Remaining: LWT for glass; `broadcast_node_status` is dead code (live node-health push not wired — REST-refresh only). |
...
5. **Theme 5 remainder** — glass LWT, live node-health WS push (`broadcast_node_status` is dead code). (N+1 `/cycles` batch endpoint ✅ done `525452e`.) Both secondary to the shipped debounce/HOL/TOCTOU/batch fixes.
