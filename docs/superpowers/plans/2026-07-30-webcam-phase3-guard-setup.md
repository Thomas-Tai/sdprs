...
**Three wiring contracts `WizardFlow` cannot enforce from inside itself.** All three are real defects if skipped, and the first two are silent:

1. **Edit mode must report the prefilled selection.** Edit mode starts with section 2 unlocked but `can_confirm` **False**, because the flow has no way to learn the saved config already has cameras ticked. A guard who opens 設定 to rename one camera would see every box ticked and a **dead 開始監控 button**. The window must call `on_cameras_selected(len(prefilled_rows))` while building its rows. Pinned by `test_edit_mode_starts_unable_to_confirm_until_the_prefill_is_reported`.
