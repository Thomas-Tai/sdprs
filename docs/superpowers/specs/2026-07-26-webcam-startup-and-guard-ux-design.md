...
#### S1–S6 結案狀態（2026-07-30 補記）

本表列出六項發現，但 §5 只寫了其中五項的作法——**S3 從稽核之後就再也沒被提過**，
Phase 1 的計畫與 `PROGRESS.md` 都沒有它的處置紀錄。補上：

| # | 狀態 |
|---|---|
| **S1** | **緩解，非修復（有意如此）。** onefile 是使用者的長期要求，不重啟討論。Payload 409.4 → 247.5 MB，啟動 39.4 → 17.2 s。 |
| **S2** | **已修復。** `SDPRS_FFMPEG` 解析 + 建置期 >120 MB 警告（`buildconfig.py:24`）。 |
| **S3** | **量測後接受，不修復。** 2026-07-30 實測：`import webcam_client.main` 確實仍會 eager import `cv2` 與 `numpy`（`main.py:9` → `gui/wizard/__init__` → `scanning` → `camera_manager`），成本為 **55–90 ms**。相對於 17.2 s 的啟動時間不到 0.5%，且這段時間 splash 已經在畫面上。改成延遲匯入要把 import 塞進函式內部、增加複雜度，而首次執行的精靈本來就立刻需要 `cv2`。**除非啟動時間的主要成本（payload 解壓）被解決，否則不值得動。** |
| **S4** | **已修復。** `Splash(...)`，0.45 s 可見。 |
| **S5** | **已修復。** `single_instance.py` mutex，且在子行程取得（onefile 雙 PID 陷阱）。 |
| **S6** | **已修復。** `main.py:494` `tray.start()` 早於 `:508` `start_engines()`，並在原始碼留下註解說明為何不可對調。 |
